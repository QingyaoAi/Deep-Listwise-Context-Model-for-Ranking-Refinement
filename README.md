...
### Data Preparation ###
    ...
    5. Generate the rank lists in the initial retrieval process using the SVMrank outputs and prepare the data for DLCM:
    	python ./scripts/Prepare_yahoo_letor_data_set1.py <letor_data_path> <inital_rank_path> <DCLM_data_path> <rank_cut>
    		<letor_data_path>: the directory of letor data. 
    		<inital_rank_path>: the directory for SVMrank outputs. 
    		<DCLM_data_path>: the directory for the inputs of DCLM. 
    		<rank_cut>: the number of top documents we keep for each query.
    
    After the data preparation, we will have the following files in <DCLM_data_path>:
    	<DCLM_data_path>/settings.json:
    		The settings we used to prepare the data

    	<DCLM_data_path>/train/:
    		1. <DCLM_data_path>/train/train.feature:
    			The feature data.
    		...
    		2. <DCLM_data_path>/train/train.init_list:
    		...
    		3. <DCLM_data_path>/train/train.gold_list:
    		...
    		4. <DCLM_data_path>/train/train.weights:
    		...
    		5. <DCLM_data_path>/train/train.initial_scores:
    		...
    		6. <DCLM_data_path>/train/train.qrels:
    		...
    		7. <DCLM_data_path>/train/train.trec.gold_list:
    		...
    		8. <DCLM_data_path>/train/train.trec.init_list:
    	...
    	<DCLM_data_path>/valid/:
            Similar to <DCLM_data_path>/train/ except that this directory is built for the validation data.

    	<DCLM_data_path>/test/:
            Similar to <DCLM_data_path>/train/ except that this directory is built for the test data.
...
### Model Training/Testing ###
    1. python ./DLCM/main.py --<parameter_name> <parameter_value> --<parameter_name> <parameter_value> … 
        ...
        11. data_dir: The data directory, which should be the <DCLM_data_path>.
        ...
        14. hparams: Hyper-parameters for models (a string written in the Tensorflow required format), which include:
            ...
            num_heads: Number of heads in the attention strategy. Default 3.
            6. l2_loss: The lambda for L2 regularization. Default 0.0
            7. att_strategy: A string that specifies the function used for attention score computation. It could be "add", "multi", "multi_add", "NTN" and "elu". Please refer to the function rnn_decoder in the source code for detailed information.
            8. use_residua: Set to True for using the initial scores to compute residua. (Not used in the final program, to be deleted)
            9. use_lstm: Set to True for using LSTM cells instead of GRU cells. Default False.
    ...
    2. Evaluation
        ...
        3. Evaluate test rank lists with ground truth <DCLM_data_path>/test/test.qrels using trec_eval or galago eval tool.
